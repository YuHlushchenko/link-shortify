...
export class NotificationsRepository {
  async getAllByUser({ userId, unreadOnly, cursor }: GetNotificationsInput): Promise<PaginatedNotifications> {
    ...
    logger.info({ text: 'getAllByUser notifications', userId, count: result.Items?.length ?? 0 })
  ...
  async getByNotificationId(userId: string, notificationId: string): Promise<NotificationItem | null> {
    const result = await docClient.send(
      new QueryCommand({
        TableName: LINKS_TABLE_NAME,
        KeyConditionExpression: 'PK = :userId AND begins_with(SK, :prefix)',
  ...
  async markAllAsRead(userId: string): Promise<void> {
    ...
    do {
      const page = await this.getAllByUser({ userId, unreadOnly: true, cursor })
    ...
    for (let retry = 1; retry <= 2; retry++) {
      ...
      retryResults.forEach((result, i) => {
        if (result.status === 'fulfilled') {
          logger.info({ text: 'markAsRead retry succeeded', retry, sk: failed[i].SK })
        } else {
