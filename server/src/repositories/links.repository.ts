...
export class LinksRepository {
  async getAll({ userId, sortBy = 'createdAt', order = 'desc', status, cursor }: GetAllLinksInput): Promise<PaginatedLinks> {
    ...
    logger.info({ text: 'getAll links', userId, count: result.Items?.length ?? 0 })
        ...
        new TransactWriteCommand({
          TransactItems: chunk.map((slug) => ({
            Update: {
              ...
              ExpressionAttributeValues: { ':status': 'deleted', ':updatedAt': now },
            },
          })),
        }),
      )
