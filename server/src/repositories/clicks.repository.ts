...
export class ClicksRepository {
  async getBySlug({ slug, from, to, cursor }: GetClicksInput): Promise<PaginatedClicks> {
    ...
    const result = await docClient.send(
      new QueryCommand({
        ...
        FilterExpression: hasRange ? 'clickedAt BETWEEN :from AND :to' : undefined,
    ...
    logger.info({ text: 'getBySlug clicks', slug, count: result.Items?.length ?? 0 })
