...
export class CreateLinkAnonValidator implements IValidator<CreateLinkAnonRequest> {
  ...
  validate(request: CreateLinkAnonRequest): void {
    ...
    if (this.errors.length > 0) {
      logger.warn({ text: "CreateLinkAnon validation failed", errors: this.errors });
  ...
  private validateFingerprint(value: unknown): void {
    if (!value || typeof value !== "string" || value.trim() === "") {
      this.errors.push({ field: "fingerprint", message: "'fingerprint' is required" });
    }
  }

  private validateOriginalUrl(value: unknown): void {
    if (!value || typeof value !== "string" || value.trim() === "") {
      this.errors.push({ field: "originalUrl", message: "'originalUrl' is required" });
      return;
    }
    ...
    try {
      ...
      this.errors.push({ field: "originalUrl", message: "'originalUrl' must be a valid URL" });
      return;
    }
  ...
  private validateExpiresAt(value: unknown): void {
    ...
    if (typeof value !== "number" || !Number.isInteger(value)) {
      this.errors.push({ field: "expiresAt", message: "'expiresAt' must be an integer Unix timestamp" });
      return;
    }

    const now = Math.floor(Date.now() / 1000);
    if (value <= now) {
      this.errors.push({ field: "expiresAt", message: "'expiresAt' must be in the future" });
    }
  }
}
